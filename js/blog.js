...
$(function(){ 
  ...
  slider();
    ...
    let white_cross = `  <i class="far fa-times-circle"></i>	`


    $('.full_screen').click(function(){

      $('#blog, #user, #slide').css({position: 'relative', opacity: 0.2});

      $('#story_modal').toggle().html($(this).parent().html()).css({top: 300, left: '20%' });
 
      $('.full_screen, .fa-times-circle').toggle();
      window.scrollTo(0, 0)
  ...
  $('#story_modal').click(function(){
    $('.full_screen, .fa-times-circle').toggle();
    $('#story_modal').toggle()
    $('#blog, #user, #slide').css({ position: "static", opacity: 1});;
  })  
